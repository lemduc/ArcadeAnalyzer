...
public class JiraClientExample {
	...
	public static void main(final String[] args) throws Exception {
		...
		final BasicCredentials creds = new BasicCredentials("batman",
				"pow! pow!");
		final JiraClient jira = new JiraClient("https://jira.example.com/jira",
				creds);
		...
		try {
			...
			issue.update().field(Field.SUMMARY, "tubes are clogged")
					.field(Field.LABELS, new ArrayList<Object>() {
						/**
				 *
				 */
			...
			issue.update().fieldAdd(Field.LABELS, "baz")
					.fieldRemove(Field.LABELS, "foo").execute();
			 ...
			 * value...
			 */
			final Object cfvalue = issue.getField("customfield_1234");

			/* ... Convert it to a string and then print the value. */
			final String cfstring = Field.getString(cfvalue);
			 ...
			 * selected values.
			 */
			final List<CustomFieldOption> cfselect = Field.getResourceArray(
					CustomFieldOption.class,
					issue.getField("customfield_5678"), jira.getRestClient());
			for (final CustomFieldOption cfo : cfselect) {
				System.out.println("Custom Field Select: " + cfo.getValue());
			}

			/* Print out allowed values for the custom multi-select box. */
			final List<CustomFieldOption> allowedValues = jira
					.getCustomFieldAllowedValues("customfield_5678", "TEST",
							"Task");
			for (final CustomFieldOption customFieldOption : allowedValues) {
				System.out.println(customFieldOption.getValue());
			}

			/* Set two new values for customfield_5678. */
			issue.update().field("customfield_5678", new ArrayList<Object>() {
				/**
				 *
				 */
				private static final long serialVersionUID = 1L;
			...
			issue.transition().field(Field.RESOLUTION, "Incomplete")
					.execute("Resolve Issue");

			/* Create a new issue. */
			final Issue newIssue = jira
					...
					.field(Field.ASSIGNEE, "robin").execute();
			...
			/* Search for issues */
			final Issue.SearchResult sr = jira.searchIssues("assignee=batman");
			System.out.println("Total: " + sr.total);
			for (final Issue i : sr.issues) {
